    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                navigate("/", { replace: true }); // ✅ Navigate Without Full Page Reload
                setShowUserLogin(false); // ✅ Hide the Login Modal
                toast.success("Welcome, " + data.user.name + "!"); // ✅ Show Success Message
            } else {
                toast.error(data.message || "Something went wrong, please try again!");
            }
        } catch (error) {
            console.error("Login Error:", error); // ✅ Log Error for Debugging
            toast.error(error.response?.data?.message || "Network issue, try again!");
        }
    };
